...
class ProviderTruck(models.Model):
    id = models.AutoField(primary_key=True, serialize=False)
    provider_id = models.ForeignKey(Provider, on_delete=models.PROTECT, null=True)
    truckMat = models.CharField('Truck License Plate', max_length=10)
    capacity = models.IntegerField('Capacity')
    truckTelephone = models.CharField('Drive Telephone', max_length=12)
    created_at = models.DateTimeField('Created At', auto_now=True, auto_now_add=False)

    def __str__(self):
         return self.truckMat
